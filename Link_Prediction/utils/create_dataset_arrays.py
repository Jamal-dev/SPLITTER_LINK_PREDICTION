...
def check_args():
    ...
    parser.add_argument("--output_dir", type=str, help="Directory where training files will be written.",
                        nargs='?', default="datasets_pp/", const="datasets_pp/")
    ...
    parser.add_argument("--input", type=str, help="Path to edge-list textfile. Required unless --only_simulate_walks is set.",
                        nargs='?', default="datasets/decompressed/CA-HepTh.txt", const="datasets/decompressed/CA-HepTh.txt")
...
def CreateDatasetFiles(graph, output_dir, num_walks,
   ...
   Returns:
    ...
    print(
        f"Writing up to {num_floats} training pairs, with size = {num_floats * 4/1000000.0} megabytes.")
...
def SimulateWalks(train_graph, output_dir, num_walks=10, walk_length=80,
    ...
    train_negatives_per_node = numpy.array(
        train_negatives_per_node, dtype='int32')
    ...
    node2vec_graph = node2vec.Graph(
        train_graph, is_directed=directed, p=p, q=q)
